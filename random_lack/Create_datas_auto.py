...
def file_path(p_,mul):
    folder_name = "lack-" + str(p_) + "_deviate_rate-" + str(mul)
...
def deviation(number):
    ...
    for i in music_info.keys():
        ...
        # score_data[music_info[i]] = score1_arr[music_info[i]]
    return score_data
...
def create_data(delta,mul):
    ...
    Folder_path,Folder_name, save_file, label_file = file_path(p_,p[1])
    # number = 2
    for i in range(100):
