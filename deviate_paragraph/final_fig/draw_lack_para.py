# -*- coding:utf-8 -*-
# @Time     : 2019-12-09 16:17
# @Author   : Richardo Mu
# @FILE     : draw_lack_para.PY
# @Software : PyCharm
...
root = os.getcwd()
print(root)
csv_file = "E:\\debug\\pyCharmdeBug\\alignment_new_2\\lack_paragraph\\csv_folder\\lack-0.2_deviate_number-[1. 0.].csv"
...
with open(csv_file,'r',newline='') as f:
    ...
    # 标出点
    plt.annotate('(0.028,'+str(accuracy_sw_ab_array[3])+")", xy=(0.028, accuracy_sw_ab_array[3]), xytext=(0.02, accuracy_sw_ab_array[3]+15), arrowprops=dict(facecolor='black', shrink=0.1))
    plt.annotate('(0.028,'+str(accuracy_sw_array[3])+")", xy=(0.028, accuracy_sw_array[3]), xytext=(0.02, accuracy_sw_array[3]-15),
                 arrowprops=dict(facecolor='black', shrink=0.01))
    name = os.path.basename(csv_file)
    name = "".join(name).split(".csv")
    print(name[0])
    # path = root + "\\"+"实验二成段漏音" + ".tiff"
    path = root + "\\" + "实验二成段漏音" + ".eps"
